...
func Error(c *gin.Context, code int, message string) {
	if core.GlobalConfig.Server.Mode == "debug" {
		log.Debugf("错误响应: %s %s → %d %s", c.Request.Method, c.Request.URL.Path, code, message)
	}
...
func Unauthorized(c *gin.Context, message string) {
	if core.GlobalConfig.Server.Mode == "debug" {
		log.Debugf("未授权: %s %s → 401 %s", c.Request.Method, c.Request.URL.Path, message)
	}
...
func Forbidden(c *gin.Context, message string) {
	if core.GlobalConfig.Server.Mode == "debug" {
		log.Debugf("禁止访问: %s %s → 403 %s", c.Request.Method, c.Request.URL.Path, message)
	}
