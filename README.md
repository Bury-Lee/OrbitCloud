...
- **账号与权限**:JWT 双令牌 + 刷新令牌轮换;注册/用户管理;十级权限模型(0 超级管理员 → 9 普通用户);用户组 + 条目级可见性 ACL
...
- `PermissionLevel` 0~9,数值越小权限越高;0 = 超级管理员,1 = 管理员,其余为普通用户
- 管理员判定统一为 `PermissionLevel <= 1`
- 权限 0 只能经命令行 `--add-superadmin` 创建,HTTP API 一律归一为普通用户
- 管理员改他人权限/状态/删除时,操作者权限须严格高于目标
- 条目级可叠加用户组 ACL(`VisibleToGroups` JSON 组 ID 列表,空 = 不限制)
